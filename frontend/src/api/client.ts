...
export const payablesApi = {
  ...
  fetchDueDates: (): Promise<{ checked: number; filled: number; no_date_in_invoice: number; failed: number; remaining: number }> =>
    api.post('/payables/fetch-due-dates').then(r => r.data),
  invoiceLineItems: (id: string): Promise<{ items: Array<{ nr: string; name: string; unit: string; qty: string; unitPrice: string; netValue: string; vatRate: string }>; error?: string }> =>
    api.get(`/payables/invoices/${id}/line-items`).then(r => r.data),
  invoiceXml: (id: string): Promise<Blob> =>
    api.get(`/payables/invoices/${id}/xml`, { responseType: 'blob' }).then(r => r.data),
